...
class ModvelModulesAdminController extends AdminTemplateController {
	...
	public function functions($functionName)
	{
		if($functionName)
		{
			if($functionName == "migrate")
			{
				Artisan::call('module:migrateall');
			}
			elseif($functionName == "scan")
			{
	...
	{
		// Class 'furkankadioglu\Modvel\Console\InvalidArgumentException' not found

		return Artisan::call('module:delete', ['moduleName' => $name]);

		$module = ModvelModule::find($id);
		$module->delete();
		return back();
	}
